...
public final class SimpleFMInst extends jm.audio.Instrument{
	...
	public SimpleFMInst(int sampleRate, int modIndex, double ratio, int channels){
		...
		System.out.println("New FM inst");
	}
	...
	*/
	public void createChain()throws AOException{
		// modulator
		modulator = new Oscillator(this, Oscillator.SINE_WAVE, this.sampleRate, this.channels);
		modulator.setFrqRatio(frqRatio);
		modulator.setAmp((float)modIndex);
		Envelope env = new Envelope(modulator, new double[] {0.0, 0.0, 0.2, 1.0, 1.0, 1.0});
	...
	public void setFrequencyRatio(double freqRatio) {
		//modulator.setFrqRatio(freqRatio);
		modulator.setFrq((float)(freqRatio * 100));
	}
