################################################################################################################
# guicontrols.py        Version 1.7     25-Nov-2014      Seth Stoudenmier, Bill Manaris, and Robert Ziehr
#
...
class Rotary(Arc):
   ...
   def paint(self, graphics2DContext):
      ...
      # draw outline arc
      graphics2DContext.setPaint(self.outline)        
      graphics2DContext.drawArc(self.startX_JPanel, self.startY_JPanel, self.endX_JPanel, self.endY_JPanel, self.startAngle, self.endAngle-self.startAngle)
...
# *** Why is (below) this different from the others? ***
   ...
   def paint(self, graphics2DContext):
      ...
      if self.state:
         graphics2DContext.setPaint(self.foreground)        
         graphics2DContext.fillRect(self.endX_JPanel * 1/6, self.endY_JPanel * 1/6, self.endX_JPanel * 13/18, self.endY_JPanel * 13/18)
      else:
...
class XYPad(Rectangle):
   ...
   def __init__(self, x1, y1, x2, y2, updateFunction = None, foreground = Color.RED, background = Color.BLACK, thickness = 3):
      """
      Create a new xy pad
      """
      
      # initialize Rectangle data and behavior
      Rectangle.__init__(self, x1, y1, x2, y2, foreground, False, thickness)
      
      # tracker width
      self.trackerWidth = 20
   ...
   def paint(self, graphics2DContext):
      ...
      # draws small center rectangle (tracker)
      graphics2DContext.setPaint(self.foreground)
      graphics2DContext.fillRect(self.xPos-self.trackerWidth/2, self.yPos-self.trackerWidth/2, self.trackerWidth, self.trackerWidth+1)
...
if __name__ == "__main__":
   ...
   t2 = Toggle(x1=200, y1=200, x2=250, y2=250, startState=False, updateFunction=printToggle, foreground=Color.BLUE, background=Color.WHITE, outline=Color.YELLOW)
   d.add(t2)
